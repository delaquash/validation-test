...
router.post('/:validate_rule',[
    ...
    check('age', "Age is required").isNumeric().not().isEmpty().withMessage('Please provide your age').isLength({ min: 1 }),
    check('position', "Kindly state your position").not().isEmpty().withMessage("Please provide your position"),
    check('missions', "Kindly state your missions value in number").isNumeric().not().isEmpty().withMessage("Kindly provide a numerical value").isLength({ min: 1 }),
    ...
    const {name, crew, age, position, missions, field, condition, condition_value} = req.body;
